...
export default function ModernTechnologiesSection() {
  ...
  return (
    <section className="w-[100hv] bg-background py-20 px-4 font-alexandria overflow-hidden">
      <div className="max-w-7xl mx-auto relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 relative z-10">
          {technologies.map((tech, index) => (
            ...
            >
              <div
                className={`absolute inset-0 bg-gradient-to-br ${tech.color} opacity-20 group-hover:opacity-30 transition-opacity duration-300`}
              />
              <div className="relative p-6 h-full flex flex-col">
                ...
                <div className="flex flex-wrap gap-2">
                  {tech.techs.map((t) => (
                    <span key={t} className="px-3 py-1 text-xs text-textMuted bg-custom-2 rounded-full">
                      {t}
                    </span>
                  ))}
                </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      {technologies
                        ...
                        ?.techs.map((t) => (
                          <div
                            key={t}
                            className="px-4 py-2 text-sm text-textPrimary bg-custom-2 rounded-lg text-center"
                          >
                            {t}
                          </div>
                        ))}
