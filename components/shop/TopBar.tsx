...
import { Instagram, Facebook, Twitter, ChevronLeft, ChevronRight, ShoppingBag, Tag, Truck, Sparkles } from "lucide-react"
...
export function TopBar() {
  ...
  // El carrusel avanza solo: el control de la derecha pasó a ser el acceso
  // directo al catálogo.
  useEffect(() => {
    const timer = setInterval(nextPromo, 6000)
    return () => clearInterval(timer)
  }, [nextPromo])

  /** Lleva al bloque de productos de la tienda. */
  ...
  return (
    <div className="w-full shop-band backdrop-blur-sm py-2.5 px-4 fixed top-0 left-0 right-0 z-[60] shadow-lg border-b border-[#08A696]/60 dark:border-[#08A696]/20">
      <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
          ...
          <div className="flex-1 overflow-hidden">
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <p className="text-xs sm:text-sm md:text-base font-semibold text-center line-clamp-2 whitespace-normal md:whitespace-nowrap overflow-hidden">
        ...
        {/* Acceso directo al catálogo - Right */}
        <div className="relative group flex-shrink-0">
          <div className="absolute -inset-0.5 bg-gradient-to-r from-[#08a6961e] to-[#26ffde23] rounded-2xl blur opacity-0 group-hover:opacity-40 transition-all duration-300" />
          <button
            onClick={scrollToProducts}
            className="relative inline-flex items-center gap-2 h-8 px-2.5 sm:px-3.5 bg-white/70 dark:bg-[#08A696]/20 backdrop-blur-sm border border-[#08A696]/80 dark:border-[#08A696]/50 rounded-2xl text-[#085c54] dark:text-[#26FFDF] text-xs sm:text-sm font-semibold transition-all duration-300 hover:border-[#08A696] dark:hover:border-[#26FFDF] hover:bg-[#08A696]/10 dark:hover:bg-[#08A696]/30 hover:shadow-lg hover:shadow-[#08A696]/20 dark:hover:shadow-[#26FFDF]/20 transform hover:scale-105 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#26FFDF]/60"
          >
            <ShoppingBag className="w-4 h-4 flex-shrink-0" />
            <span className="whitespace-nowrap">Ver productos</span>
            <ChevronRight className="w-4 h-4 flex-shrink-0 transition-transform duration-300 group-hover:translate-x-0.5" />
          </button>
        </div>
      </div>
    </div>
