...
export default function CardBanner({ icon, title, onClick, href }: CardBannerProps) {
  ...
  const cardContent = (
    <>
      ...
      ></div>
      <div
        className={`relative ${isDark ? "bg-[#02505931] backdrop-blur-sm" : "bg-[#e6f7f6] backdrop-blur-sm"} p-6 rounded-2xl border ${isDark ? "border-[#08A696]/20" : "border-[#08A696]/30"} flex flex-row items-center justify-center transition-all duration-300 transform scale-95 group-hover:scale-100 group-hover:border-[#08A696] ${isDark ? "group-hover:bg-[#02505950]" : "group-hover:bg-[#c5ebe7]"} min-h-[140px] h-full shadow-lg group-hover:shadow-xl group-hover:shadow-[#08A696]/10`}
      >
        <div className="w-1/5 flex items-center justify-center mr-4 transition-transform duration-300 group-hover:scale-110">
          <div className={`p-3 rounded-xl ${isDark ? "bg-[#08A696]/10" : "bg-[#08A696]/5"} group-hover:bg-[#08A696]/20 transition-all duration-300`}>
            <Image src={icon || "/placeholder.svg"} alt="" height={60} width={60} className="transition-transform duration-300 group-hover:rotate-3" />
          </div>
        </div>
        <h3 className={`w-4/5 ${isDark ? "text-[#26FFDF]" : "text-[#08A696]"} font-semibold text-lg leading-tight transition-colors duration-300 group-hover:text-[#08A696] ${isDark ? "group-hover:text-[#26FFDF]" : ""}`}>
          {title}
        </h3>
      </div>
    </>
